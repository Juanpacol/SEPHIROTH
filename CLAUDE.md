...
src/sephiroth/runtime/executor.py
    ↓
intent_router picks ONE specialist → it answers directly.  1 LLM call
    ↓
...
`LabAgent`/`ClinicalCoordinator` and the multi-agent fan-out (`enable_single_agent_mode`/`enable_dynamic_planner`) were removed in Phase 14 (`SPEC-029`, `ADR-016`) — see decision #24.
...
24. **One specialist answers — no coordinator turn.** `src/sephiroth/runtime/intent_router.py` routes every consultation to exactly ONE specialist, whose answer IS the final answer — since Phase 14 (`SPEC-029`, `ADR-016`) this is the only path, not a default: `settings.enable_single_agent_mode`/`enable_dynamic_planner`, `COORDINATOR`, `LABORATORY`, and the multi-agent fan-out (`planner.py`, `router.py`) were removed as unreachable-in-production dead code (the flag had defaulted `True` with nothing else setting it since it landed). This cut a consultation from 4-8 sequential model round-trips to 3 (1 answer + 2 verification), which is what makes the chat usable on a free/local model — measured: every free provider tried (Groq's 8000 TPM account cap, OpenRouter's saturated free models, local Ollama at ~23 tok/s) made the old multi-agent path take 90s-3m43s. The router is three tiers, cheapest first: keyword rules on the question (zero latency, resolves most real phrasing), then structured context signals (`analyze()`), then one `generate_json` classification — degrading to `evidence` at every failure. Rules match on **intent, not topic**: "What A1C goal is appropriate?" names a lab test but asks for a guideline, so `evidence` is checked first and wins. `laboratory`-shaped questions (e.g. "interpret this elevated potassium") fall through the same way, landing on `evidence` by default — the deterministic curated lab rules that used to live in the `laboratory` agent's prompt now live in `sephiroth.safety.risk`, surfaced outside the chat path entirely. Everything downstream — citation guard, claim verification, abstention, trace — is unaffected; the closing disclaimer is appended deterministically by `executor._with_disclaimer` rather than trusted to a smaller model's instruction-following.
...
6. **Citation Guard on every answer.** `src/sephiroth/verification/citation_guard.py` (relocated verbatim from `intelligence/agents/citation_guard.py` in Phase 5; that shim was deleted in Phase 6) audits the answering specialist's final answer against actual tool output; fabricated citations are stripped (`[unverified — removed]`) and reported in `citation_report` (shown in the UI). Since Phase 4 it's a pre-filter feeding claim-level verification, not the terminal check — see #15.
...
16. **Each agent sees only the context fields it declares.** `AgentCapability.context_fields` (`src/sephiroth/runtime/registry.py`) names which `RunContext` fields an agent needs; `src/sephiroth/context/views.py::context_for_agent` projects down to just those before the executor calls it. "Memory" is scoped narrowly to a patient's own recent consultations (`src/sephiroth/context/memory.py`, injected by the router into `context["recent_consultations"]`) and reaches **only the agent that writes the final answer** — the one specialist `intent_router` selected (`context_for_agent(..., answering=True)`, which adds `recent_consultations` and nothing else, so memory widens without clinical-data access widening). Not a generic multi-turn chat session, which doesn't exist in this product yet (see ADR-011).
17. **Every consultation builds a replayable trace, persisted alongside it.** `src/sephiroth/telemetry/build_trace` projects the executor's `RunState` into `ExecutionTrace` (`sephiroth.contracts.trace`) at the end of a run; `traced_span` records real timing spans around each agent turn and the verification pass, redacted via an attribute allow-list (never patient content). Toggled by `settings.enable_tracing` — disabling it must not change anything else about a run's result (ADR-009). `ExecutionTrace.tokens`/`.cost_usd` are real, not placeholders (SPEC-006 v1.2.0/SPEC-016): `GeminiClient`/`GroqClient` report actual `prompt_tokens`/`completion_tokens` on `ChatResult`, summed per agent, and priced through `src/sephiroth/telemetry/pricing.py`'s hand-maintained table (`0.0` for an unrecognized model). `RunState.coordinator_result` has had no live producer since Phase 14 (`SPEC-029` removed the coordinator) — kept only because removing the field would be an unrelated breaking change to `sephiroth.contracts`. `generate_json` calls (claim extraction, verification) still report no usage — a known, smaller gap.
...
19. ~~Routing can be model-driven, behind a flag.~~ **Removed in Phase 14** (`SPEC-029`, superseding `SPEC-008`). `route_specialists_dynamic`/`enable_dynamic_planner` asked the model which specialists to fan out to — a question that stopped applying once a consultation always routes to exactly one specialist (decision #24). `intent_router`'s own LLM tier (keyword/context tiers exhausted) is the only model-driven routing left, and it always resolves to a single name.
...
26. **Commit messages must match `SF<NNN> <body, ≤50 words> [<type>]`, where `<NNN>` is the user story/feature id shared by every commit in that story.** Applies from `SF001` onward; prior history keeps its old Conventional-Commits style, never rewritten. Enforced two ways — a local `commit-msg` hook (`git config core.hooksPath .githooks`) for fast feedback, and a required `commit-lint` CI job on every PR against `main` that's the real, unbypassable gate — both calling the same `scripts/validate_commit_msg.py` so they can't drift apart. Story-id uniqueness across branches is explicitly NOT enforced (ADR-015) — `scripts/next-sf-id.sh` only suggests the next id. Dependabot-authored commits are exempt from the format — by subject prefix (`dependabot: `) in the validator, and by author email in the `commit-lint` job, since a Dependabot security-update PR may not carry the configured prefix. See `CONTRIBUTING.md`.

27. **A perception tool's own output can ground a claim without corroborating it.** `VerificationStatus.OBSERVED` (SPEC-004 1.2.0, ADR-018) is assigned deterministically — never by the LLM judge — when a claim's cited evidence is entirely `harvest_observations`-built (vision/imaging tool output, `source_type=TOOL_OUTPUT`), kept separate from `harvest_evidence` so the anti-self-verification rule holds. `grounded_claim_ratio` weights it at `OBSERVED_WEIGHT=0.6` (lives in `contracts/claims.py`, not `verification/`, to keep `contracts` a leaf package — `verification/confidence.py` imports and re-exports it), landing an all-OBSERVED answer inside the `partial` confidence band with a distinct `OBSERVED_BANNER`. This fixed a real bug: a RadiologyAgent answer faithful to a real vision-model finding always harvested zero evidence and abstained on `INSUFFICIENT_EVIDENCE`, discarding a correct answer. An invented (tool-unreported) high-risk finding still abstains, unchanged.
...
2. Give `intent_router.py` a way to route to it (a keyword rule, a context
   signal, or rely on its LLM tier, which already enumerates `AGENTS`) —
   there is no separate planner since Phase 14 (`SPEC-029`)
