...
An **AI-powered clinical decision support platform** for healthcare professionals. Patients upload histories, imaging, and lab data; specialized AI agents (running on a local model by default, or Gemini if configured) extract findings, retrieve evidence from medical literature, check drug interactions, and generate structured, cited recommendations.
...
⚠️ **Privacy:** with the default configuration (`LLM_PROVIDER=ollama`, `AI_ALLOW_PHI=false`) no clinical content leaves the machine. Point it at Gemini (`LLM_PROVIDER=gemini`) and clinical text and medical images are sent to the Google Gemini API; the AI Studio free tier may use submitted data to improve Google's models, and `AI_ALLOW_PHI` must additionally be turned on before the PHI-carrying features will send anything. Neither configuration is a HIPAA/GDPR compliance claim — running locally removes one specific exposure, not the programme around it.

## Tech Stack

- **LLM**: Ollama by default (`qwen2.5:14b`, local, no key). Optionally Google Gemini (`gemini-flash-latest`, free AI Studio tier) with `LLM_PROVIDER=gemini`, or Groq as a text/tool-calling fallback behind Gemini (`GROQ_API_KEY`). All four sit behind one `ModelProvider` protocol.
...
| Folder | Purpose |
|---|---|
| `platform/api/` | FastAPI routes, grouped by domain since SPEC-028: `clinical/routers/` (patients, encounters, alerts, results, portal, dashboard), `operations/routers/` (scheduling, tasks, approvals, followups, automation-memory, badges, notifications, push, internal), `intelligence/routers/` (agents, rag, medical), `security/routers/` (audit) — plus `services/`, `workflows/`, `main.py` (CORS/lifespan), and the shared `audit.py`/`fast_path.py`/`paging.py`/`timeparse.py`/`scheduling.py`/`pdf_export.py` |
...
1. **Local by default, cloud by explicit choice (SPEC-022).** `llm_provider` defaults to **`ollama`**: a fresh install runs against a local model and sends nothing anywhere. It used to default to `gemini`, which meant an operator who configured no provider at all was shipping clinical text to a third party by omission — the one default a clinical product must not have. Gemini is still fully supported and is what the public demo runs; it now requires `LLM_PROVIDER=gemini` said out loud. A second switch, `ai_allow_phi` (default `false`), decides whether patient content may reach a *non-local* provider at all: it is inert while the provider is local, and when it bites, the affected features degrade deterministically (consultations refuse with the real provider named, drafts keep their template, timelines fall back to the lexicon) rather than 503-ing. It gates an enumerated list of call sites, not a payload classifier — see `docs/08-decisions/ADR-015-phi-egress-enumerated-seams.md` for why, and `docs/05-operations/on-premise-runbook.md` for the clinic deployment. Every endpoint that names a provider or model (`/health`, `/health/ready`, `/api/agents/status`) reads it from the client's own `describe()`; all three used to print `settings.gemini_*` regardless of what was running.
...
1. Create router in `platform/api/<domain>/routers/my_feature.py` — `clinical/` for
   patient-facing clinical work, `operations/` for the inbox/scheduling/automation,
   `intelligence/` for the agent runtime and its lookups, `security/` for audit-only
   concerns. Its own imports of `services`/`workflows`/`audit`/`paging`/`timeparse`
   should be absolute (`from api.services import ...`), not relative — the domain
   folder adds a level of nesting the shallower routers never had (SPEC-028).
...
- **A model that is unreachable degrades gracefully, doesn't crash.** `/health/ready` reports `llm: unreachable` while keeping `status: ready` (losing the model costs features; losing the database costs the instance), a consultation that needs no model still answers via the fast path, follow-up drafts keep their deterministic template, timeline extraction falls back to a lexicon, and vision returns `status: "unavailable"`. A consultation that genuinely needs a model returns 503 naming the *real* provider and model — the message used to blame Gemini regardless.
