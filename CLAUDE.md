...
26. **Commit messages must match `SF<NNN> <body, ≤50 words> [<type>]`.** Applies from `SF001` onward; prior history keeps its old Conventional-Commits style, never rewritten. Enforced two ways — a local `commit-msg` hook (`git config core.hooksPath .githooks`) for fast feedback, and a required `commit-lint` CI job on every PR against `main` that's the real, unbypassable gate — both calling the same `scripts/validate_commit_msg.py` so they can't drift apart. `<NNN>` uniqueness across branches is explicitly NOT enforced (ADR-015) — `scripts/next-sf-id.sh` only suggests the next id. See `CONTRIBUTING.md`.
