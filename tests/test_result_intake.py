...
Verifies AC-024-01, AC-024-09 (docs/specs/SPEC-024-results-loop.md). AC-024-03
(a critical/abnormal result becomes a task) is task-inbox territory —
`TestWorkCreated` and every task-filing assertion elsewhere in this file were
dropped rather than kept as tests that could never pass; see
`result_service._file_task`'s docstring for why.
"""
...
from data.schemas import LabResult, Patient, ResultReview
...
class TestWorkFilesNoTaskYet:
    """AC-024-03's task-inbox half is deferred (see this module's docstring):
    a review is still created and classified for every case that would
    deserve a task, so the inbox itself (`list_reviews`, sorted by severity)
    already surfaces it -- what's missing is only the separate task-list
    entry until the task inbox is restored."""

    async def test_a_critical_result_files_no_task(self, db_session):
        patient = await _patient(db_session)
...
class TestThePanelStaysInStep:
    ...
    async def test_the_risk_engine_now_sees_what_was_recorded(self, db_session):
        ...
        patient = await _patient(db_session, lab_results={}, medications=[])
        await svc.record_lab_result(
            db_session, patient_id=patient.id, test_name="potassium", value=6.2, now=NOW
        )
        await db_session.flush()
        ...
        # `LabRule` yet; `label` is the field both versions have.
        assert any(flag["label"] == "Hyperkalemia" for flag in flags)
...
class TestImagingIntake:
    ...
    async def test_a_critical_study_is_classified_but_files_no_task_yet(self, db_session):
        patient = await _patient(db_session)

        _study, review, _c = await svc.record_imaging_study(
            db_session,
            patient_id=patient.id,
            modality="TAC",
            body_part="cráneo",
