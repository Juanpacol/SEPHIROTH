...
def test_rag_tools_are_async():
    ...
    can no longer be plain sync functions."""
    import asyncio

    assert asyncio.iscoroutinefunction(rag_server.search_clinical_guidelines)
    assert asyncio.iscoroutinefunction(rag_server.list_evidence_categories)
    assert asyncio.iscoroutinefunction(rag_server.list_evidence_by_category)
