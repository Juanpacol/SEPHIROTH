...
class TestLifecycle:
    ...
    async def test_signing_files_no_task_until_the_task_inbox_is_restored(self, client, headers, patient):
        """SPEC-018's unified task inbox was removed by the c7fbdf5 revert;
        `encounter_service._file_orders` is a deliberate no-op until it comes
        back (see that module's docstring), so signing an order today leaves
        it unfiled rather than raising."""
        ...
        assert res.json()["tasks_created"] == []
        assert res.json()["orders"][0]["task_id"] is None
