...
def test_fallback_extracts_entities_with_note_date():
    events = _fallback_extract("Patient diagnosed with hypertension, started lisinopril.", "2026-01-15")
...
async def test_extract_events_falls_back_when_llm_fails():
    events = await extract_events(BrokenClient(), "Patient with diabetes on insulin.", "2026-02-01")
