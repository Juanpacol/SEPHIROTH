...
resolved is not filed again: that was a live defect, and SPEC-020 made it worse
by turning `alert_refresh` from once-per-deploy into every six hours.
...
class TestAlertsDoNotRepeat:
    ...
    async def test_a_resolved_alert_may_be_raised_again(self, db_session):
        """Resolved means the clinician dealt with it. If the condition is
        still true on the next sweep, that is new information, not a duplicate."""
