...
def _title(review: ResultReview, label: str) -> str:
    if review.severity == "critical":
        return f"Resultado crítico sin revisar: {label}"
    if review.severity == "unclassified":
        return f"Resultado sin rango de referencia: {label}"
    return f"Resultado anormal sin revisar: {label}"
...
async def _file_task(
    ...
    *,
    ...
    """One task per result that is worth someone's attention.

    Keyed on the review, so re-running intake for the same result finds the
    task it already made rather than filing a second one.
    """
    severity = TASK_SEVERITY_BY_RESULT.get(review.severity)
    if severity is None:
        return None
    task, _created = await svc.create_task(
        ...
        source_id=review.id,
        category="lab" if review.result_type == "lab" else "imaging",
        severity=severity,
        patient_id=review.patient_id,
        title=_title(review, label),
        detail=detail,
        dedupe_key=f"result_review:{review.id}",
        context={"result_type": review.result_type, "result_id": review.result_id},
        now=now,
    )
...
async def close_result(
    ...
    *,
    ...
    """Close the loop, and the work it raised.
    ...
    if review.task_id:
        ...
        task = await session.get(Task, review.task_id)
        if task is not None and task.status in svc.OPEN_STATUSES:
            await svc.transition(session, task, "complete", actor=actor, now=moment)
    return review
