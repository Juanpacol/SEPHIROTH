...
`ClinicalNote` that reaches the chart and files one task per order. Before it,
an encounter is invisible to the rest of the product — which is what makes the
signature, rather than a separate approval row, the review gate for AI-drafted
content (ADR-016).

Transaction discipline: **nothing here commits.** Same rule as
`task_service.py`; the routers commit.
"""
...
#: An order's kind, and the task category it becomes. The inbox already has
#: these categories (SPEC-018), so an order lands next to the work it belongs
#: with rather than in a category of its own.
ORDER_TASK_CATEGORY = {
    "lab": "lab",
    "imaging": "imaging",
    "referral": "followup",
...
#: How urgent the resulting task is. An order with a deadline carries its own
#: due date, so severity here is about how it sorts in a list, not when it is
#: due -- a referral is not more urgent than a lab, it is just less routine.
ORDER_TASK_SEVERITY = {
    "lab": "medium",
    "imaging": "medium",
    "referral": "medium",
...
async def _file_orders(
    ...
    for order in orders:
        if order.task_id is not None:
            continue
        due_at = now + timedelta(days=order.due_in_days) if order.due_in_days else None
        task, _created = await svc.create_task(
            session,
            source_type="encounter",
            source_id=order.id,
            category=ORDER_TASK_CATEGORY[order.kind],
            severity=ORDER_TASK_SEVERITY[order.kind],
            patient_id=encounter.patient_id,
            title=_ORDER_TITLE.get(order.kind, "Orden de consulta"),
            detail=order.detail,
            # Keyed on the order row, so signing twice -- or a retry after a
            # failed commit -- finds the task it already made.
            dedupe_key=f"encounter:{encounter.id}:{order.id}",
            context={"encounter_id": encounter.id, "order_kind": order.kind},
            # Passed as `due_at`, not `source_deadline`: the latter only wins
            # for the `approval` category (`sla.py::CATEGORY_USES_SOURCE_DEADLINE`)
            # and would be silently ignored here. A deadline a clinician set in
            # the room outranks a severity default, so it is the due date.
            # Without one, `create_task` falls back to the SLA.
            due_at=due_at,
            now=now,
        )
