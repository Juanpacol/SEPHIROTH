...
Relocated verbatim from `intelligence/agents/explainability.py` (Phase 5,
`docs/00-migration-charter.md`'s shim schedule). Deliberately deterministic
and template-based (no extra LLM call): the whole point is trustworthiness,
so this layer must not add another hallucination surface. It is derived on
demand from data the workflow already records (agents involved, tagged
tool calls, citation report) — nothing is persisted. Kept as its own
function taking the executor's already-assembled (agents, tool_calls,
citation_report) rather than reimplemented as an `ExecutionTrace`
projection this cycle — that redesign is deferred, not assumed free.
"""
