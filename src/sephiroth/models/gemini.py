...
Sends chat requests to the Google Gemini API using its native
``function_declarations`` tools. When the model responds with function calls,
they are executed through the caller-supplied executor and the results are fed
back as ``functionResponse`` parts until the model produces a plain assistant
answer.

Moved from `intelligence/llm/gemini_client.py` in Phase 1
(`docs/specs/SPEC-001-model-provider.md`) to implement `ModelProvider`. Shared
rate limiting and retry backoff now live in `sephiroth.models._throttle`
instead of being duplicated inline; `describe_image` reuses `_generate`
(parameterized by model) instead of carrying its own copy of the retry loop.
...
class GeminiClient:
    ...
    def __init__(
        ...
        self._sleep = sleep
        self._rate_limiter = RateLimiter(rpm_limit, time_source=time_source, sleep=sleep)
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if exc.code == 429 and attempt < self.max_retries - 1:
                    delay = backoff_delay(attempt, cap=_BACKOFF_CAP_SECONDS)
    ...
    async def describe_image(
        ...
        Delegates to `_generate` with `model=self.vision_model`, so the retry
        loop and rate limiter are shared with `chat`/`generate_json` rather
        than duplicated.
        """
        contents = [
            types.Content(
