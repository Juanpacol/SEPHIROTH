...
Relocated verbatim from `intelligence/agents/citation_guard.py` (Phase 5,
`docs/00-migration-charter.md`'s shim schedule). Deliberately kept as its
own module, not merged into `claims.py`/`evidence.py`: this module audits
citation *labels* against tool output (regex/token matching), a distinct
job from claim-level content verification. It still feeds the executor
as a pre-filter, per ADR-006 — this move only changes where the code
lives, not what it does.

Local models occasionally fabricate citations (observed in testing: an
`[UpToDate]` reference no tool ever returned). This module harvests the
set of legitimate citations from the executed tool calls, audits the
final answer, and strips anything that cannot be traced back to a tool
result.
"""
