...
**Watching tick health:** `GET /api/dashboard/automation` (as a clinician)
shows `workflows.total` and per-step status directly. Also turn on
cron-job.org's own failure alert (its execution settings → notify on
failure/non-200, by email): if the cron stops firing entirely or Render is
down, that's the only signal that catches it, since nothing in this app can
