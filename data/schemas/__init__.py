...
class ResultShare(Base):
    ...
    #: Set by `result_service.communicate_result` (SPEC-024) so a
    #: `ResultReview` can answer "was this communicated" via its `share_id`
    #: without a second sharing mechanism. Nullable: a share made before
    #: SPEC-024, or one made outside the review loop, has neither.
...
class EncounterOrder(Base):
    ...
    `task_id` is a plain column, not yet a foreign key to `tasks` — the
    unified task inbox (SPEC-018) is a later restoration phase. The `tasks`
    table already exists in the database (this schema trails production,
    which never dropped it), so the column stays compatible; the ORM-level FK
    comes back with that phase rather than importing `Task` early.
    """
...
class ResultReview(Base):
    ...
    `task_id` is a plain column, not yet a foreign key to `tasks` — same
    restoration-phase note as `EncounterOrder.task_id` above.
    """
